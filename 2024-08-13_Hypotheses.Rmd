...
julia_command("h1_fm_excl_triplet_item = fit(LinearMixedModel, h1_fm_excl_triplet_item, jl_data_dwelltime, optimizer = NLopt.Optimizer(:LN_BOBYQA) do opt
                opt.maxeval = 10000
            end)")
...
opt = NLopt.Options(:LN_BOBYQA)
opt.maxeval = 10000  # Set maximum evaluations

# Define the model formula
h1_fm_excl_triplet_item = @formula(dwell_diff ~ group + stim_ocd + stim_pho + stim_neg +
                                   group & stim_ocd + group & stim_pho + group & stim_neg +
                                   (1 | participant))

# Fit the mixed model with custom optimization options
model = fit(MixedModel, h1_fm_excl_triplet_item, data, optimizer = opt)
...
julia_command("opt = Opt(:LN_BOBYQA, 2)")  # Adjust '2' to the number of variables in your problem
julia_command("opt.maxeval = 10000")
...
data_dwelltime_ <- data_dwelltime %>%
  dplyr::mutate(stim_ocd = as.factor(stim_ocd),
                stim_pho = as.factor(stim_pho),
                stim_neg = as.factor(stim_neg))
modeltest <- lmer(dwell_diff ~ group + stim_ocd + stim_pho + stim_neg + (1 + stim_ocd + stim_pho + stim_neg | participant),
     data = data_dwelltime_, control = lmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb")))
...
# Fit the model with a higher maxeval for the optimizer
model = fit(MixedModel, h1_fm_excl_triplet_item, data,
            optimizer = NLopt.Optimizer(:LN_BOBYQA) do opt
                opt.maxeval = 10000
            end)
