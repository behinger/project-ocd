...
# Determine which stimulus (lower-left or upper-right) was fixated first and store the corresponding stimulus ID
...
rownames_h1_contrast <- c("stim_ocd + groupocd:stim_ocd","(stim_ocd + groupocd:stim_ocd) - (stim_neg + groupocd:stim_neg)", "groupocd + groupocd:stim_ocd", "(groupocd + stim_ocd + groupocd:stim_ocd) - (groupspider + stim_pho + groupspider:stim_pho)")
...
OCD patients do not dwell sig. longer at idiosyncratic OCD-relevant material as compared to neutral pictures. 

OCD patients do not dwell sig. longer at idiosyncratic OCD-relevant material as compared to negative pictures. 

OCD patients do not show a sig. more pronounced maintenance bias on idiosyncratic OCD-relevant material as compared to healthy controls on the same material (rated by matched OCD patient).

OCD patients do show a sig. more pronounced maintenance bias on idiosyncratic OCD-relevant material as compared to spider phobia patients on idiosyncratic spider-phobia material.
...
# Testing Convergence Issue of H1 With Julia
```{julia h1 mimic in julia - step 1 set up model}
# Define model
h1_fm_excl_triplet_item = @formula(dwell_diff ~ group + stim_ocd + stim_pho + stim_neg + group&stim_ocd + group&stim_pho + group&stim_neg + 
              (1 + stim_ocd + stim_pho + stim_neg | participant))

# Calculate model
h1_fm_excl_triplet_item = fit(LinearMixedModel, h1_fm_excl_triplet_item, jl_data_dwelltime)
```


```{julia h1 mimic in julia - step 2 check convergence}
println(h1_fm_excl_triplet_item)
```


```{julia h1 mimic in julia - step 2 check convergence 2}
println(h1_fm_excl_triplet_item.optsum)
```
*Converged successfully*


```{julia h1 mimic in julia - contrast coding}
Pkg.add("DataFrames")
Pkg.add("PrettyTables")
Pkg.add("Distributions")
using DataFrames, MixedModels, LinearAlgebra, PrettyTables

# Extract fixed effects and covariance matrix
fixed_effects = coefnames(h1_fm_excl_triplet_item)         # Names of fixed effects
coefficients = fixef(h1_fm_excl_triplet_item)             # Fixed effect estimates
covariance_matrix = vcov(h1_fm_excl_triplet_item)         # Covariance matrix of fixed effects


# Define contrasts
contrast_matrix = [
    Dict("stim_ocd" => 1, "group: ocd & stim_ocd" => 1),
    Dict("stim_ocd" => 1, "group: ocd & stim_ocd" => 1, 
         "stim_neg" => -1, "group: ocd & stim_neg" => -1),
    Dict("group: ocd" => 1, "group: ocd & stim_ocd" => 1),
    Dict("group: ocd" => 1, "stim_ocd" => 1, "group: ocd & stim_ocd" => 1,
         "group: spider" => -1, "stim_pho" => -1, "group: spider & stim_pho" => -1)
]

# Compute contrasts
contrast_results = []
for contrast in contrast_matrix
    contrast_vector = zeros(length(fixed_effects))  # Initialize contrast vector
    for (name, weight) in contrast
        index = findfirst(==(name), fixed_effects)  # Find index of name in fixed_effects
        if index !== nothing
            contrast_vector[index] = weight
        else
            println("Warning: Term not found in fixed effects: ", name)
        end
    end

    # Calculate contrast value and standard error
    contrast_value = dot(coefficients, contrast_vector)
    contrast_variance = transpose(contrast_vector) * covariance_matrix * contrast_vector
    contrast_se = sqrt(contrast_variance)

    # Store results
    push!(contrast_results, (contrast_value, contrast_se))
end

# Create a DataFrame for the results
contrast_names = ["Stim OCD vs Neutral (OCD Group)", 
                  "Stim OCD vs Negative (OCD Group)",
                  "OCD vs Healthy on Stim OCD",
                  "OCD vs Spider on Stim Disorder Relevant"]

using Distributions
results_df = DataFrame(
    Contrast = contrast_names,
    Estimate = [result[1] for result in contrast_results],
    SE = [result[2] for result in contrast_results],
    t_value = [result[1] / result[2] for result in contrast_results],
    p_value = [2 * (1 - cdf(Normal(0, 1), abs(result[1] / result[2]))) for result in contrast_results]
)

Pkg.add("CSV")
using CSV
# Round values for better presentation
for col in [:Estimate, :SE, :t_value, :p_value]
    results_df[!, col] = round.(results_df[!, col], digits=3)
end
...
CSV.write("/Volumes/109-psy-kp-study/044_EYE/06_Results/02_Hauptstudie/TryOut/Models_and_Contrasts_with_Julia/h1_contrasts.csv", results_df)
```
...
Spider phobia patients are sig. more likely to first fixate at idiosyncratic spider-relevant material as compared to neutral pictures. 

Spider phobia patients are sig. more likely to first fixate at idiosyncratic spider-relevant material as compared to negative pictures. 
...
Healthy participants are sig. more likely to first fixate spider-relevant stimuli compared to neutral stimuli.

Healthy participants are sig. more likely to first fixate spider-relevant stimuli compared to negative stimuli.

# Testing Convergence Issue of H2a With Julia

```{julia h2a mimic in julia - step 1 set up model}
# Define model
h2a_glm_fm_excl_triplet_item = @formula(response ~ group + stim_ocd + stim_pho + stim_neg + group&stim_ocd + group&stim_pho + group&stim_neg + 
              (1 + stim_ocd + stim_pho + stim_neg | participant))

# Calculate model
h2a_glm_fm_excl_triplet_item = fit(LinearMixedModel, h2a_glm_fm_excl_triplet_item, jl_data_entrytime)
```


```{julia h2a mimic in julia - step 2 check convergence }
println(h2a_glm_fm_excl_triplet_item)
```


```{julia h2a mimic in julia - step 2 check convergence 2}
println(h2a_glm_fm_excl_triplet_item.optsum)
```


```{julia h2a mimic in julia - contrast coding}
Pkg.add("DataFrames")
Pkg.add("PrettyTables")
Pkg.add("Distributions")
using DataFrames, MixedModels, LinearAlgebra, PrettyTables
...
# Extract fixed effects and covariance matrix
fixed_effects = coefnames(h2a_glm_fm_excl_triplet_item)    # Names of fixed effects
coefficients = fixef(h2a_glm_fm_excl_triplet_item)        # Fixed effect estimates
covariance_matrix = vcov(h2a_glm_fm_excl_triplet_item)    # Covariance matrix of fixed effects

# Define directional contrasts
directional_contrasts = [
    Dict("stim_ocd" => 1, "group: ocd & stim_ocd" => 1),
    Dict("stim_ocd" => 1, "group: ocd & stim_ocd" => 1, 
         "stim_neg" => -1, "group: ocd & stim_neg" => -1),
    Dict("stim_pho" => 1, "group: spider & stim_pho" => 1),
    Dict("stim_pho" => 1, "group: spider & stim_pho" => 1, 
         "stim_neg" => -1, "group: spider & stim_neg" => -1)
]

# Define equality contrasts
equality_contrasts = [
    Dict("stim_ocd" => 1),
    Dict("stim_ocd" => 1, "stim_neg" => -1),
    Dict("stim_pho" => 1),
    Dict("stim_pho" => 1, "stim_neg" => -1)
]

# Function to compute contrasts
function compute_contrasts(contrasts, fixed_effects, coefficients, covariance_matrix)
    results = []
    for contrast in contrasts
        contrast_vector = zeros(length(fixed_effects))
        for (name, weight) in contrast
            index = findfirst(==(name), fixed_effects)
            if index !== nothing
                contrast_vector[index] = weight
            else
                println("Warning: Term not found: ", name)
            end
        end
        value = dot(coefficients, contrast_vector)
        variance = transpose(contrast_vector) * covariance_matrix * contrast_vector
        se = sqrt(variance)
        push!(results, (value, se))
    end
    return results
end

# Compute directional contrasts
directional_results = compute_contrasts(directional_contrasts, fixed_effects, coefficients, covariance_matrix)

# Compute equality contrasts
equality_results = compute_contrasts(equality_contrasts, fixed_effects, coefficients, covariance_matrix)

# Create DataFrames
using Distributions

directional_names = [
    "Stim OCD vs Neutral (OCD Group)", 
    "Stim OCD vs Negative (OCD Group)", 
    "Stim Pho vs Neutral (Spider Group)", 
    "Stim Pho vs Negative (Spider Group)"
]

equality_names = [
    "Stim OCD = 0 (Healthy Group)", 
    "Stim OCD - Stim Neg = 0 (Healthy Group)", 
    "Stim Pho = 0 (Healthy Group)", 
    "Stim Pho - Stim Neg = 0 (Healthy Group)"
]

h2a_dir_results_df = DataFrame(
    Contrast = directional_names,
    Estimate = [result[1] for result in directional_results],
    SE = [result[2] for result in directional_results],
    t_value = [result[1] / result[2] for result in directional_results],
    p_value = [1 - cdf(Normal(0, 1), result[1] / result[2]) for result in directional_results]  # One-tailed
)

h2a_eq_results_df = DataFrame(
    Contrast = equality_names,
    Estimate = [result[1] for result in equality_results],
    SE = [result[2] for result in equality_results],
    t_value = [result[1] / result[2] for result in equality_results],
    p_value = [2 * (1 - cdf(Normal(0, 1), abs(result[1] / result[2]))) for result in equality_results]  # Two-tailed
)
...
# Round values for better presentation
for df in [h2a_dir_results_df, h2a_eq_results_df]
    for col in [:Estimate, :SE, :t_value, :p_value]
...
# Print the tables
println("Directional Contrasts:")
pretty_table(h2a_dir_results_df)

println("Equality Contrasts:")
pretty_table(h2a_eq_results_df)

# Save the results to CSV
CSV.write("/Volumes/109-psy-kp-study/044_EYE/06_Results/02_Hauptstudie/TryOut/Models_and_Contrasts_with_Julia/h2a_directional_contrasts.csv", h2a_dir_results_df)
CSV.write("/Volumes/109-psy-kp-study/044_EYE/06_Results/02_Hauptstudie/TryOut/Models_and_Contrasts_with_Julia/h2a_equality_contrasts.csv", h2a_eq_results_df)
```
...
OCD patients are not sig. faster to first fixate at idiosyncratic OCD-relevant material as compared to neutral pictures. 

OCD patients are not sig. faster to first fixate at idiosyncratic OCD-relevant material as compared to negative pictures. 

Patients with spider phobia are sig. faster to first fixate at idiosyncratic spider-relevant material as compared to neutral pictures. 

Patients with spider phobia are sig. faster to first fixate at idiosyncratic spider-relevant material as compared to negative pictures. 

Healthy participants are not sig. faster to first fixate at OCD-relevant stimuli compared to neutral stimuli.

Healthy participants are not sig. faster to first fixate at OCD-relevant stimuli compared to negative stimuli.

Healthy participants are not sig. faster to first fixate spider-relevant stimuli compared to neutral stimuli.

Healthy participants are not sig. faster to first fixate spider-relevant stimuli compared to negative stimuli.
...
# Testing Convergence Issue of H2b With Julia
```{julia h2b mimic in julia - step 1 set up model}
# Define coding
c_rt = Dict(:group=>DummyCoding(;base="healthy"),:looked_right=>EffectsCoding(),:looked_at=>DummyCoding(;base="neutral"))

# Define model

h2b_fm_excl_triplet_item = @formula(rt ~ group*looked_at + looked_right*group + 
              (1 + looked_at + looked_right | participant))

# Calculate model
h2b_fm_excl_triplet_item = fit(LinearMixedModel, h2b_fm_excl_triplet_item, jl_data_entrytime; contrasts=c_rt)
```


```{julia h2b mimic in julia - step 2 check convergence}
println(h2b_fm_excl_triplet_item)
```


```{julia h2b mimic in julia - step 2 check convergence 2}
println(h2b_fm_excl_triplet_item.optsum)
```


```{julia h2b mimic in julia - contrast coding}
Pkg.add("DataFrames")
Pkg.add("PrettyTables")
Pkg.add("Distributions")
using DataFrames, MixedModels, LinearAlgebra, PrettyTables
...
# Extract fixed effects and covariance matrix
fixed_effects = coefnames(h2b_fm_excl_triplet_item)         # Names of fixed effects
coefficients = fixef(h2b_fm_excl_triplet_item)             # Fixed effect estimates
covariance_matrix = vcov(h2b_fm_excl_triplet_item)         # Covariance matrix of fixed effects

# Define contrasts for directional hypotheses
contrast_matrix_directional = [
    Dict("looked_at: ocd" => 1, "group: ocd & looked_at: ocd" => 1),  # OCD-relevant vs Neutral (OCD Group)
    Dict("looked_at: ocd" => 1, "group: ocd & looked_at: ocd" => 1, 
         "looked_at: neg" => -1, "group: ocd & looked_at: neg" => -1),  # OCD-relevant vs Negative (OCD Group)
    Dict("looked_at: pho" => 1, "group: spider & looked_at: pho" => 1),  # Spider-relevant vs Neutral (Spider Group)
    Dict("looked_at: pho" => 1, "group: spider & looked_at: pho" => 1, 
         "looked_at: neg" => -1, "group: spider & looked_at: neg" => -1)  # Spider-relevant vs Negative (Spider Group)
]

# Compute contrasts for directional hypotheses
contrast_results_directional = []
for contrast in contrast_matrix_directional
    contrast_vector = zeros(length(fixed_effects))  # Initialize contrast vector
    for (name, weight) in contrast
        index = findfirst(==(name), fixed_effects)  # Find index of name in fixed_effects
        if index !== nothing
    ...
    # Store results
    push!(contrast_results_directional, (contrast_value, contrast_se))
end

# Define contrasts for equality hypotheses
contrast_matrix_equality = [
    Dict("looked_at: ocd" => 1),  # OCD-relevant = Neutral (Healthy Group)
    Dict("looked_at: ocd" => 1, "looked_at: neg" => -1),  # OCD-relevant = Negative (Healthy Group)
    Dict("looked_at: pho" => 1),  # Spider-relevant = Neutral (Healthy Group)
    Dict("looked_at: pho" => 1, "looked_at: neg" => -1)  # Spider-relevant = Negative (Healthy Group)
]

# Compute contrasts for equality hypotheses
contrast_results_equality = []
for contrast in contrast_matrix_equality
    contrast_vector = zeros(length(fixed_effects))  # Initialize contrast vector
    for (name, weight) in contrast
        index = findfirst(==(name), fixed_effects)  # Find index of name in fixed_effects
        if index !== nothing
    ...
    # Store results
    push!(contrast_results_equality, (contrast_value, contrast_se))
end

# Create DataFrames for results
using Distributions
contrast_names_directional = ["OCD-relevant vs Neutral (OCD Group)",
                              "OCD-relevant vs Negative (OCD Group)",
                              "Spider-relevant vs Neutral (Spider Group)",
                              "Spider-relevant vs Negative (Spider Group)"]

contrast_names_equality = ["OCD-relevant = Neutral (Healthy Group)",
                           "OCD-relevant = Negative (Healthy Group)",
                           "Spider-relevant = Neutral (Healthy Group)",
                           "Spider-relevant = Negative (Healthy Group)"]

h2b_dir_results_df = DataFrame(
    Contrast = contrast_names_directional,
    Estimate = [result[1] for result in contrast_results_directional],
    SE = [result[2] for result in contrast_results_directional],
    t_value = [result[1] / result[2] for result in contrast_results_directional],
    p_value = [2 * (1 - cdf(Normal(0, 1), abs(result[1] / result[2]))) for result in contrast_results_directional]
)



h2b_eq_results_df = DataFrame(
    Contrast = contrast_names_equality,
    Estimate = [result[1] for result in contrast_results_equality],
    SE = [result[2] for result in contrast_results_equality],
    t_value = [result[1] / result[2] for result in contrast_results_equality],
    p_value = [2 * (1 - cdf(Normal(0, 1), abs(result[1] / result[2]))) for result in contrast_results_equality]
)
...
# Round values for better presentation
for col in [:Estimate, :SE, :t_value, :p_value]
    h2b_dir_results_df[!, col] = round.(h2b_dir_results_df[!, col], digits=3)
    h2b_eq_results_df[!, col] = round.(h2b_eq_results_df[!, col], digits=3)
end

# Print the tables
pretty_table(h2b_dir_results_df)
pretty_table(h2b_eq_results_df)

# Save the results to CSV
CSV.write("/Volumes/109-psy-kp-study/044_EYE/06_Results/02_Hauptstudie/TryOut/Models_and_Contrasts_with_Julia/h2b_directional_contrasts.csv", h2b_dir_results_df)
CSV.write("/Volumes/109-psy-kp-study/044_EYE/06_Results/02_Hauptstudie/TryOut/Models_and_Contrasts_with_Julia/h2b_equality_contrasts.csv", h2b_eq_results_df)
```
